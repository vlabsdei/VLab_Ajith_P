...
<p>Every watt a propulsion system fails to turn into shaft power comes out as heat, and heat is what ultimately limits how hard you can fly a drone. A motor does not fail because it runs out of torque, it fails because its winding enamel melts. An ESC does not stop switching because it runs out of current headroom, it browns out because its MOSFETs cook. This experiment treats the motor and the ESC as <strong>thermal systems</strong>: bodies that make heat internally from resistive loss, store it in their own mass, and shed it to the surrounding air through a thermal resistance the propeller's own downwash can change. The chapter works through the four things that decide whether a build survives a sustained full-throttle hold: how much heat the copper actually makes, what steady temperature that heat settles at, how long the part takes to get there, and how much cooling airflow buys back.</p>

<blockquote>
<p><strong>A note on the build used throughout this chapter.</strong> Every worked example below uses the same bench build: a 2310 2400&nbsp;KV motor (33&nbsp;g, <i>R</i><sub>m,20&deg;C</sub> = 0.060&nbsp;&Omega;), a 3&Prime; tri-blade propeller, a Cyclone 35A ESC (<i>R</i><sub>DS(on)</sub> = 0.0028&nbsp;&Omega;), and a 4S 1500&nbsp;mAh pack, run at full throttle in still air with an ambient of <i>T</i><sub>amb</sub> = 25&nbsp;&deg;C and no cooling parts fitted. Every number comes from the equations below using the same constants the simulator's solver uses, so the figures match what the bench readout reports for this build.</p>
</blockquote>

<p>Before we get into the physics, the animation below walks through the full thermal story: the copper loss that makes the heat and the two-body RC network that stores and sheds it, then the steady-state temperature that loss settles at and the time constant that decides how long the part takes to get there, and finally the ESC's own thermal limit and how much headroom forced cooling buys back.</p>

<p align="center">
  <video controls playsinline preload="metadata" width="100%" style="max-width: 860px; border-radius: 8px;">
    <source src="./videos/thermal_management.mp4" type="video/mp4">
    Your browser does not support the HTML5 video tag. You can
    <a href="./videos/thermal_management.mp4">download the video</a> instead.
  </video>
</p>
...
<p>The main heat source in a running BLDC motor is <strong>resistive (copper, or <i>I</i><sup>2</sup><i>R</i>) loss</strong> in the stator windings. Every amp the ESC pushes through the phase resistance dumps power as heat:</p>

<p align="center"><i>P</i><sub>cu</sub> = <i>I</i><sup>2</sup> &middot; <i>R</i><sub>m</sub>(<i>T</i>)</p>

<p>The winding resistance is not constant. Copper's resistivity climbs with temperature, so a hot motor is a <em>worse</em> motor, dissipating more for the same current. The copper temperature coefficient <i>&alpha;</i><sub>Cu</sub> = 0.00393&nbsp;/&deg;C captures this:</p>

<p align="center"><i>R</i><sub>m</sub>(<i>T</i>) = <i>R</i><sub>m,20&deg;C</sub> &middot; [1 + <i>&alpha;</i><sub>Cu</sub>(<i>T</i> &minus; 20)]</p>

<p>Because <i>R</i><sub>m</sub> rises with <i>T</i>, and <i>T</i> rises with <i>P</i><sub>cu</sub>, and <i>P</i><sub>cu</sub> rises with <i>R</i><sub>m</sub>, the three quantities are coupled. The simulator solves them together as a fixed point, exactly as a real motor settles to a self-consistent operating temperature. When a winding is badly over-propped for its size and there is no cooling, this positive feedback has no stable fixed point below the enamel-failure ceiling of <strong>240&nbsp;&deg;C</strong>; the model then reports <strong>thermal runaway</strong> and treats the winding as burnt out.</p>

<h3>The Lumped-Capacitance Model &amp; the Two-Node RC Analogy</h3>

<p>A body that makes heat internally and loses it to ambient behaves like an electrical RC circuit, with an exact one-to-one analogy:</p>
...
<p>The simplest model lumps the whole motor into one node: heat <i>P</i><sub>cu</sub> flows into a thermal mass <i>C</i><sub>th</sub> and leaks to ambient through <i>R</i><sub>th</sub>. In reality the copper winding runs hotter than the aluminium case a sensor can actually touch, so the simulator uses a research-grade <strong>two-node network</strong>: a fast <em>winding</em> node (capacitance <i>C</i><sub>w</sub>, where the heat is made) conducts through a fixed winding-to-housing resistance <i>R</i><sub>wh</sub> = 5&nbsp;&deg;C/W into a slow <em>housing</em> node (<i>C</i><sub>h</sub>), which finally convects and radiates to ambient through the airflow-dependent <i>R</i><sub>th,eff</sub>. The winding hotspot is then <em>emergent</em>: it falls straight out of the network instead of being added on by hand.</p>
...
<p>Leave a part running at a constant load long enough and every transient dies away and it reaches a <strong>steady state</strong>: the heat made exactly equals the heat shed, and the temperature stops climbing. Set the net heat flow to zero and the case (housing) temperature is just the ambient plus the loss driven across the effective thermal resistance, with the winding hotspot one more <i>R</i><sub>wh</sub> drop above that:</p>
...
<p>The <strong>thermal time constant</strong> <i>&tau;</i> is thermal resistance times thermal capacitance, and it fixes the timescale of the whole heating curve. The most useful physical reading of <i>&tau;</i> is the <strong>63.2%-of-rise crossing</strong>: at <i>t</i> = <i>&tau;</i> the part has climbed 63.2% of the way from ambient to its final steady temperature; by 3<i>&tau;</i> it is 95% there, and by 5<i>&tau;</i> it is effectively settled. A large <i>&tau;</i> is protective: a heavy motor with a big <i>C</i><sub>th</sub> can soak up a punishing burst for tens of seconds before it gets anywhere near its steady temperature, which is why short aerobatic bursts are survivable where the same current held indefinitely would not be.</p>

<p>The lumped thermal capacitance is the part's mass times the specific heat of its main material, copper's <i>c</i><sub>p</sub> = 385&nbsp;J/(kg&middot;&deg;C) for the winding:</p>
...
<p align="center"><i>&tau;</i> &asymp; <i>R</i><sub>th,eff</sub> &middot; <i>C</i><sub>th</sub> = 9.171 &middot; 10.74 &asymp; <b>98&nbsp;s</b></p>
<p>Integrate the true two-node case-temperature curve to its measurable 63.2% crossing and you get <i>&tau;</i> &asymp; <b>114&nbsp;s</b>, a little above the simple product, because the winding node has to charge before the housing node can. Either way the motor takes on the order of two minutes to approach its steady 64&nbsp;&deg;C case temperature; a five-second full-throttle punch barely moves the needle.</p>
</blockquote>
...
<p>The ESC has its own thermal story. Its MOSFET bridge makes heat by <strong>conduction loss</strong> as phase current flows through the on-resistance of the switching FETs, with a soft penalty that grows once the current nears the board's rated limit (rising switching and reverse-recovery losses):</p>
...
<p>That heat settles the ESC to a steady temperature through its own thermal resistance, <i>T</i><sub>esc</sub> = <i>T</i><sub>amb</sub> + <i>P</i><sub>esc</sub>&middot;<i>R</i><sub>th,esc</sub>, and the board is judged against a hard <strong>over-temperature limit of <i>T</i><sub>esc,limit</sub> = 80&nbsp;&deg;C</strong>. Below it the board is fine; above it the FET junctions are past their safe operating area and the ESC is flagged as failed. Because the ESC's aluminium PCB and FET packages have a high specific heat (<i>c</i><sub>p</sub> &asymp; 800&nbsp;J/kg&middot;&deg;C), it has a meaningful time constant of its own.</p>

<blockquote>
<p><strong>Worked example: ESC temperature on the bench build.</strong>
On this single-motor 3&Prime; bench point the Cyclone 35A ESC's conduction loss is small next to its 35&nbsp;A rating, so it settles at <i>T</i><sub>esc</sub> = <b>27.9&nbsp;&deg;C</b>, only 2.9&nbsp;&deg;C above ambient and comfortably under the 80&nbsp;&deg;C limit. Re-run the same board on a hard-pulling 6S build near its current ceiling and the (<i>I</i>/<i>I</i><sub>limit</sub>)<sup>2</sup> penalty term takes over, driving the ESC past 80&nbsp;&deg;C and tripping the survivability check, the classic reason a correctly-sized motor still needs a correctly-sized (or actively cooled) ESC behind it.</p>
</blockquote>
...
<p><strong>Bolt-on cooling parts.</strong> A heatsink, thermal pad, or ducted fan mounted on the motor or ESC each cuts the local <i>R</i><sub>th</sub> by a fixed fraction, and they <em>stack multiplicatively</em> on top of the airflow reduction above:</p>
...
<p>The effective thermal resistance is nearly halved, which directly halves the steady-state temperature rise: the same 4.283&nbsp;W of copper loss now lifts the case only 4.283&middot;4.81 &asymp; 20.6&nbsp;&deg;C, to about 45.6&nbsp;&deg;C instead of 64.3&nbsp;&deg;C. Cooling does not reduce the heat made, it reduces the temperature that heat settles at, and that is often the whole difference between a build that survives a sustained hold and one that cooks.</p>
</blockquote>
