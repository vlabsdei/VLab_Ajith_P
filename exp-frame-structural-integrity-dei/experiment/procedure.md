...
2. **Select Hardware Size**:
   * Under the **1. Select Frame** section in the left panel, choose a wheelbase (e.g., $450\text{ mm}$). This defines the deck dimensions and the length of the diagonal arms.
   * Select a **Battery Pack** (e.g., LiPo 4S $4500\text{ mAh}$) and a **Payload** (e.g., Action Camera with Gimbal) from the option tiles.
3. **Locate Initial CG**:
   ...
   * Observe the **2D CG Offset Envelope** scatter plot in the right panel. The center green circle represents the $10\text{ mm}$ safe balancing boundary. Note that the target CG dot is outside this circle, causing the 3D model of the drone in the viewport to tilt.
...
5. **Lock the Balance Point**:
   * Adjust the coordinates until the **CG Center Offset** checklist item turns green (meaning the offset magnitude is less than $10\text{ mm}$).
...
2. **Setup Test Profile**:
   * In the left panel, select **Carbon Fibre T700** under the materials section, and select the **$150\text{ mm}$** arm length.
...
4. **Populate the 3x3 Safety Matrix**:
   ...
   * Repeat the stress sweep for all **9 combinations** (3 materials: Carbon Fibre, Aluminium, Nylon × 3 lengths: $150\text{ mm}$, $250\text{ mm}$, $350\text{ mm}$).
   * Note the structural behavior: Nylon arms will yield and **snap** ($SF < 1.0$) when tested at longer lengths.
