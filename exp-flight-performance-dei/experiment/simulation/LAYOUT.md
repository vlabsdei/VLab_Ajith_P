# Drone Technology Lab — Experiment 06 — Layout & Architecture

Research-grade virtual drone laboratory: **Thrust-to-Weight Ratio, Hover
Analysis & Payload Capacity Envelope**, forked from the Drone Technology Lab
Platform. **Stack: HTML + CSS + vanilla JS only** (libraries: three.js r128
with OrbitControls / GLTFLoader / FBXLoader, Chart.js 4.4.1).
...
│  · Payload    │  (Auto/Manual + Forward      │               │
│    slider     │   Flight 30° toggle, hover)  │               │
│  · Mass       ├──────────────────────────────┼───────────────┤
...
## 5 · Physics model (propulsion + Exp-6 sub-calcs, real-lab grade)
...
- Payload slider (0–500 g) folds straight into the mass budget as a synthetic
  row, so it drives T/W, hover throttle and every chart live.
- Exp-6 sub-calcs (all in `calc()`): `controlMargin = 100 − hoverPct` (paper
  margin); `effMargin` compresses that margin for throttle nonlinearity near
  full stick; `payloadTheoMax_g` / `payloadPracMax_g` = max cargo at TWR 1.0 /
  1.5; `twEff` derates T/W by cos(30°) when the Forward Flight toggle is on.

### Modules & experiments
| Module | Experiment | Simulation |
|---|---|---|
| M1 Lift-Off Budget | Mass Budget & TWR | thrust-stand spin-up, go/no-go verdict at T/W thresholds 1.0 / 1.5 |
| M1 | Thrust Efficiency | throttle sweep 0→100 %, motor η and g/W vs a 5 g/W benchmark |
| M2 Flight Envelope | Hover & Control Margin | live RK4 hover to target altitude; records hover throttle % and control margin vs payload |
| M2 | Payload Capacity | swept internal cargo 0→past TWR 1.0, live T/W vs cargo, verdict against the practical (1.5) / theoretical (1.0) envelope |

## 6 · Persistence (localStorage `dtl-exp6`)

Component selections, altitude, payload, cosine-loss toggle, module/experiment
tabs, completed experiments, instructor step, volumes. Progress-only — no
cross-experiment reads/writes.
